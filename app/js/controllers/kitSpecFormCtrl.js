four51.app.controller('KitSpecFormCtrl', ['$scope', '$location', '$route', '$routeParams', '$window', 'ProductDisplayService', 'Variant', 'Order',
        ...
        function saveVariant(variant, saveNew, hideErrorAlert /*for compatibility*/) {
            ...
            Variant.save(variant, function(data){
                if ($scope.isEditforApproval || $scope.EditingLineItem) {
                    ...
                    $location.path('/kit/' + $routeParams.id + '/' + $routeParams.lineitemid + '/' + $routeParams.productInteropID + '/'+ data.InteropID);
                }
            });
        }
