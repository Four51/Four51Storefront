four51.app.directive('customtextfield', function($451) {
    var obj = {
        ...
        restrict: 'E',
        transclude: true,
        template: '<label>{{customfield.Name}}</label><input class="form-control" ng-if="customfield.Lines <= 1" placeholder="{{customfield.Name}}" size="{{customfield.Width * .13}}" max="{{customfield.MaxLength}}" ui-mask="{{customfield.MaskedInput}}" type="text" ng-required="{{customfield.Required}}" ng-model="customfield.Value"></input>' +
